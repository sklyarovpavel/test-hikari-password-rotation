...
update_file() {
  ...
  sed -E -i '/^[[:space:]]*datasource:[[:space:]]*$/,/^[^[:space:]]/ s|(^[[:space:]]*username:[[:space:]]*).*$|\1app_user_b|' "$f"
  sed -E -i '/^[[:space:]]*datasource:[[:space:]]*$/,/^[^[:space:]]/ s|(^[[:space:]]*password:[[:space:]]*).*$|\1app_pass_b|' "$f"
  echo "Updated credentials in $f -> app_user_b"
}
